...
const SQL = `SELECT key, original_filename, tg_file_id, tg_status, tg_error, is_public, folder_id FROM images WHERE key = ?`;
...
export const onRequestGet: PagesFunction<Env> = withRequestLogging('/api/image/:key/original', async ({ env, params }, logger) => {
  ...
  try {
    const row = await env.DB.prepare(SQL).bind(key).first<PublicOriginalRow>();
